/*
  RoboPaint RT "BB" -- A Clock application for use with the
 original Buddha Board (http://www.buddhaboard.com), 
 for water painting the time in a clock face.
...
import java.util.*;  // for time utilities
...
int minDist = 2; // Minimum drag distance to record
...
//Clock size constants:
int paperCenterX = int ((MousePaperLeft + MousePaperRight)/2);
int paperCenterY = int ((MousePaperTop + MousePaperBottom)/2);
float clockRadius =  0.9 * (paperCenterY - MousePaperTop);
int clockCenterY = paperCenterY - int( (paperCenterY  - clockRadius)/8  );
int clockCenterX = paperCenterX;
float minuteRadius = 0.8 * clockRadius;
float hourRadius = 0.5 * minuteRadius;
...
void setup() 
{
  ...
  keepingTime = true; 
  minuteLast = 500; // Force redraw
  millisecsLast = 0;// MILLISECOND
}
...
void draw() {
  ...
  int hour = getHourNow();
  int second = getSecondNow(); 
  int minute = getMinuteNow();
  int millisecs = getMillisNow();// milliseconds portion of current date/time-- not millis().
  int xTemp, yTemp;
  int posOld, posNew;
  int i;  // I don't always use dummy variables, but when I do, I like to make it obvious.
  ...
  if (keepingTime) {
    if (minute != minuteLast)
    {
        ...
        println("paperCenterY: " + paperCenterY ); 
      println("clockRadius: " + clockRadius ); 
      println("clockCenterY: " + clockCenterY ); 
      println("(paperCenterY  - clockRadius)/2: " + (paperCenterY  - clockRadius)/2 ); 

      //    Once a minute, clear all and redraw the time
...
//        pause();
      ...
      ToDoList = append(ToDoList, -30); // Command Code:  -30 (raise brush)  (If the brush is already down.)
      getWater(0, false);   // Brush is clean.  Dip in the clean water.

      println("hour: " + getHourNow() );
      println("minute: " + getMinuteNow() );
      println("second: " + getSecondNow() );

      //    println("minuteRadius: " + minuteRadius );
      //    println("hourRadius: " + hourRadius ); 

      float minuteAngle = (float(minute) / 60) * TWO_PI - HALF_PI; 
      if (hour == 12)
      { 
        hour = 0;
      }
      float hourAngle = (((float(minute)/60) + float(hour))/12) * TWO_PI - HALF_PI;


      // DRAW MINUTE HAND:::
      xTemp = clockCenterX + int (minuteRadius * cos(minuteAngle));
      yTemp = clockCenterY + int (minuteRadius * sin(minuteAngle));

      ToDoList = append(ToDoList, xyEncodeInt2(clockCenterX, clockCenterY));    // Command Code: Move to paper center
      ToDoList = append(ToDoList, -31);              // Command Code:  -31 (lower brush)
      appendFilledSegment(clockCenterX, clockCenterY, xTemp, yTemp, 5);
      ToDoList = append(ToDoList, -30); // Command Code:  -30 (raise brush)  (If the brush is already down.)
      ...
      // DRAW HOUR HAND:::
      xTemp = clockCenterX + int (hourRadius * cos(hourAngle));
      yTemp = clockCenterY + int (hourRadius * sin(hourAngle));

      ToDoList = append(ToDoList, -30); // Command Code:  -30 (raise brush)  (If the brush is already down.)
      ToDoList = append(ToDoList, xyEncodeInt2(clockCenterX, clockCenterY));    // Command Code: Move to first (X,Y) point
      ToDoList = append(ToDoList, -31);              // Command Code:  -31 (lower brush)
      appendFilledSegment(clockCenterX, clockCenterY, xTemp, yTemp, 5);
      ToDoList = append(ToDoList, -30); // Command Code:  -30 (raise brush)  (If the brush is already down.)

      // DRAW CLOCK FACE (COULD BE USED AS A SECOND HAND?):::   

      //      secondAngle = ((float(second)  + (float(millisecs)/1000.0)) / 60.0) * TWO_PI - HALF_PI; 

      // Start at minute-hand position.
      xTemp = clockCenterX + int (clockRadius * cos(minuteAngle));
      yTemp = clockCenterY + int (clockRadius * sin(minuteAngle));
      ToDoList = append(ToDoList, xyEncodeInt2(xTemp, yTemp));    // Command Code: Move to (X,Y) point
      ToDoList = append(ToDoList, -31);              // Command Code:  -31 (lower brush)
      ...
      while (i <= steps)
      {
        ...
        minuteAngle += TWO_PI / steps;  
        xTemp = clockCenterX + int (clockRadius * cos(minuteAngle));
        yTemp = clockCenterY + int (clockRadius * sin(minuteAngle));


        ToDoList = append(ToDoList, xyEncodeInt2(xTemp, yTemp));    // Command Code: Move to first (X,Y) point
        i++;
      }
    ...
    { 
      /*
       ...
       // Draw clock face circle to indicate seconds?  
       if (millisecsLast != millisecs)
       {
       secondAngle = ((float(second)  + (float(millisecs)/1000.0)) / 60.0) * TWO_PI - HALF_PI; 
       
       xTemp = clockCenterX + int (clockRadius * cos(secondAngle));
       yTemp = clockCenterY + int (clockRadius * sin(secondAngle));
       
       i = ToDoList.length; 
       
       posNew = xyEncodeInt2(xTemp, yTemp);
       ...
       float distTemp = getDistance(posOld, posNew) ;
...
void keyPressed()
{
  ...
  else
  {
    ...
    if ( key == 'h')  // display help
    {
      ...
      // Debug: Print hour, minute, second:
      println("hour: " + getHourNow() );
      println("minute: " + getMinuteNow() );
      println("second: " + getSecondNow() );
    }
